...
public class StudentControllerTest {
	...
	public void shouldReturnGetStudentListFromService() throws Exception {
		//Arrange
		when(studentService.getStudentList())
				.thenReturn(StudentServiceImpl.STUDENT_DATA);
		//Act
		this.mockMvc.perform(get("/v1/student"))
				.andDo(print())
	...
	public void shouldReturnGetStudentSuccessfull() throws Exception {
		...
		when(studentService.getId(2L))
			.thenReturn(StudentServiceImpl.STUDENT_DATA.get(0));
		
		this.mockMvc.perform(get("/v1/student/2"))
			.andDo(print())
			.andExpect(status().isOk())
			.andExpect(jsonPath("[1].id").value(StudentServiceImpl.STUDENT_DATA.get(1).getId()))
			.andExpect(jsonPath("[1].name").value(StudentServiceImpl.STUDENT_DATA.get(1).getName()))
			.andExpect(jsonPath("[1].surName").value(StudentServiceImpl.STUDENT_DATA.get(1).getSurName()))
			.andExpect(jsonPath("[1].age").value(StudentServiceImpl.STUDENT_DATA.get(1).getAge()))
			.andExpect(jsonPath("[1].courses").value(StudentServiceImpl.STUDENT_DATA.get(1).getCourses()));
	...
	public void shouldReturnDataNotFoundExceptionFromGetStudent() throws Exception {
		...
		this.mockMvc.perform(get("/v1/student/100"))
		.andDo(print())
